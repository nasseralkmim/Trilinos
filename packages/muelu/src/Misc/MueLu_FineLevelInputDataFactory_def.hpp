...
void FineLevelInputDataFactory<Scalar, LocalOrdinal, GlobalOrdinal, Node>::Build(Level& currentLevel) const {
  ...
  if (variableType == "int") {
    ...
    if (variableType.find("Aggregates") != npos) {
      ...
      typedef Kokkos::DynRankView<LocalOrdinal, typename Node::device_type> FieldContainer;
      RCP<FieldContainer> data = currentLevel.Get<RCP<FieldContainer> >(variableName, fact.get());
      Set(currentLevel, variableName, data);
    }
#endif
    else {
